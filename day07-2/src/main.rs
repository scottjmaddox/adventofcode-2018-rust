...
fn main() {
    ...
    let mut time_remainings = [0; 5];
    let mut time = 0;
    loop {
        for i in 0..5 {
            if workers[i].is_some() {
                time_remainings[i] -= 1;
                if time_remainings[i] == 0 {
                    let c = workers[i].take().unwrap();
                    completed.push(c);
        ...
        for i in 0..5 {
            if workers[i].is_none() && !ready.is_empty() {
                ready.sort_unstable_by(|a, b| b.cmp(a));
                let c = ready.pop().unwrap();
                let time_required = 60 + (u32::from(c) as usize - 64);
                workers[i] = Some(c);
                time_remainings[i] = time_required;
            }
        }
        if completed.len() == inv_deps.len() {
            break;
        }
        time += 1;
    }
